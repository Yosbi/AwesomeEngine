...
Waves::Waves(int m, int n, float dx, float dt, float speed, float damping)
{
	...
	for (int i = 0; i < m; ++i)
	{
		...
		for (int j = 0; j < n; ++j)
		{
			...
			mPrevSolution[i * n + j] = AWEVector(x, 0.0f, z);
			mCurrSolution[i * n + j] = AWEVector(x, 0.0f, z);
			mNormals[i * n + j] = AWEVector(0.0f, 1.0f, 0.0f);
			mTangentX[i * n + j] = AWEVector(1.0f, 0.0f, 0.0f);
		}
	}
}
		...
		concurrency::parallel_for(1, mNumRows - 1, [this](int i)
			//for(int i = 1; i < mNumRows - 1; ++i)
			{
				for (int j = 1; j < mNumCols - 1; ++j)
				{
					...
					mTangentX[i * mNumCols + j] = AWEVector(2.0f * mSpatialStep, r - l, 0.0f);
